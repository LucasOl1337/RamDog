...
<img src="docs/screenshot-main.png" alt="Janela do RamDog em visão Lista: medidores de CPU, RAM, GPU e Disco no topo, chips de categoria (IA/Agentes, Dev, Navegador, Jogos, Pessoal, Sistema, Outros), grupo de ícones dos addons ao lado das abas (Partida, Desperdício, Térmico, Telas — os dois acesos têm janela aberta) e a tabela agrupada por app: brave.exe e chrome.exe recolhidos numa linha só com 26 e 24 processos, somando RAM, CPU, GPU e disco no cabeçalho do grupo, e o painel de detalhes explicando o Memory Compression." width="100%">
...
- **Visões.** Lista (plana), Árvore (pai → filhos, RAM da subárvore), Categorias (agrupado). Os addons saem das abas e viram um grupo de ícones ao lado delas — **Partida**, **Desperdício**, **Térmico** e **Telas**, só no Windows. Cada um abre em **janela própria**, com seu tamanho e sua posição, e o RamDog reabre no dia seguinte o que você deixou aberto.
...
| Visões | **Lista**, **Árvore**, **Categorias** nas abas de cima; **Partida**, **Desperdício**, **Térmico** e **Telas** no grupo de ícones logo à direita (só no Windows) — cada clique abre ou fecha a janela daquele addon, o ícone aceso diz qual está aberto e o nome está no tooltip |
...
Aba **Desperdício** (Windows): leitura direta (SCM + registro, sem PowerShell). **Ação** usa Win32 direto quando o RamDog já está elevado; senão dispara um PowerShell elevado — **um prompt de UAC por ação**. No macOS a aba existe só para dizer isso.
...
Aba **Telas** (Windows): o mapa desenha os monitores na proporção real, com a resolução de cada um e ★ no primário.
...
- Telas: `EnumDisplayMonitors`, `EnumWindows`, `SetWindowPos` e o DWM (para descontar a sombra invisível da janela). No macOS o equivalente é a Accessibility API, que exige permissão explícita do sistema — a aba existe só para dizer isso.
