...
- **Visões.** Lista (plana), Árvore (pai → filhos, RAM da subárvore), Categorias (agrupado) nas abas ao lado da busca. Os addons — **Partida**, **Desperdício**, **Térmico**, **Telas** e **Limpeza** — ficam longe delas, com o nome escrito no bloco de controles do canto superior direito: clicar troca o conteúdo da janela, clicar de novo volta para a lista de processos. No Linux: Partida e Desperdício usam systemd/XDG, Telas usa Hyprland, Térmico usa hwmon com controle autenticado de PWM e Limpeza junta RAM (sobras, zombies, apps parados em segundo plano) com disco (`~/.cache`, lixeira, pacman, journal, coredumps, órfãos). Veja [o porte Linux](linux/README.md).
