...
- Térmico: sensores hwmon, RPM e controle PWM manual/Auto/ESTABILIZAR em controlador compatível. A interface normal não precisa rodar como root; `sudo -n` quando já autorizado, senão `pkexec`, eleva somente o helper. ESTABILIZAR controla apenas fans conectados com RPM positivo (CPU e gabinete); bomba e headers vazios ficam na BIOS. Manual limitado a 30–100%, proteção térmica e restauração dos modos/PWM anteriores quando o pai termina. O app precisa permanecer aberto para manter a curva; fechado, a BIOS assume. Não controla fans da GPU.
- Faxina (só Linux): agrupa cada app com os processos dele e classifica pelo que o RamDog observou de CPU, disco, GPU e foco. **Pode fechar** vem marcado e só recebe sobras com evidência (emulador escondido; processo aberto por um agente cuja sessão já saiu, parado há 10 min e sem janela). **Talvez**: sem janela e parado há 30 min, ou janela sem foco há 2 h. **Em uso**: atividade ou foco recentes, serviço do systemd ou auxiliar de app em uso. Marca e desmarca por grupo ou linha, fecha com confirmação levando os filhos, e nunca lista protegidos nem sistema. "Manter sempre" guarda a identidade do app, não o nome do runtime. A observação fica em `sweep.json` ao lado da config e sobrevive a reinícios de até 15 min; com o RamDog recém-aberto, quase tudo aparece em uso.
- Limpeza (só Linux): RAM e disco numa tela. Em cima, `/proc/meminfo` (uso, cache do kernel, swap) e "Soltar cache do kernel" (`sync` + `drop_caches` + `compact_memory`, via `pkexec`). No meio, os zombies: não recebem sinal, então cada um aponta o pai com "encerrar pai". Apps abertos sem uso ficam na Faxina. Embaixo, o disco: cada subpasta de `~/.cache` (`XDG_CACHE_HOME`) acima de 1 MB, lixeira (`XDG_DATA_HOME/Trash`), cache do pacman (`paccache -rk1` + `-ruk0`), journal (`--vacuum-size=64M`), coredumps e órfãos (`pacman -Qtdq` → `-Rns`). Tudo pede confirmação inline; o que é do sistema roda em `ramdog --clean-helper <op>` como root via `pkexec`, sem receber caminho nem nome de pacote por argumento (o helper recalcula). Symlinks nunca são seguidos, nem no tamanho nem na remoção.
