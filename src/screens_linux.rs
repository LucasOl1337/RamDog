...
fn selector_for(address: &str, locale: Locale) -> Result<String, String> {
    ...
    {
        return Err(locale
            .text("Endereço de janela inválido", "Invalid window address")
            .into());
    }
...
fn lua_dispatch_for(action: &str, arg: &str, locale: Locale) -> Result<String, String> {
    ...
    let window = |s: &str| -> Result<String, String> {
        selector_for(
            s.strip_prefix("address:")
                .ok_or(locale.text("Seletor inválido", "Invalid selector"))?,
            locale,
        )?;
        Ok(quote(s))
    };
    Ok(match action {
        ...
        "movewindow" => {
            let (monitor, w) = arg
                .split_once(',')
                .ok_or(locale.text("Monitor inválido", "Invalid monitor"))?;
            format!(
        ...
        "resizewindowpixel" | "movewindowpixel" => {
            let (coords, w) = arg
                .split_once(',')
                .ok_or(locale.text("Coordenadas inválidas", "Invalid coordinates"))?;
            let fields: Vec<_> = coords.split_whitespace().collect();
            if fields.len() != 3 || fields[0] != "exact" {
                return Err(locale
                    .text("Coordenadas inválidas", "Invalid coordinates")
                    .into());
            }
        ...
        _ => {
            return Err(locale
                .text("Dispatcher não suportado", "Unsupported dispatcher")
                .into())
        }
    })
}
...
pub fn place_for(
    ...
    {
        return Err(locale
            .text("Nome de monitor inválido", "Invalid monitor name")
            .into());
    }
    if !rect.is_finite() || rect.width() < 1.0 || rect.height() < 1.0 {
        return Err(locale
            .text("Retângulo inválido", "Invalid rectangle")
            .into());
    }
    ...
    dispatch_for(
        "movewindow",
        &format!("mon:{},{}", monitor.name, window),
        locale,
    )?;
    dispatch_for(
        "resizewindowpixel",
...
pub fn apply_for(preset: ScreenPreset, locale: Locale) -> Result<(), String> {
    ...
    for slot in preset.slots {
        ...
        let monitor = layout
            ...
            .ok_or(locale.text(
                "Monitor do cenário indisponível",
                "Scene monitor is unavailable",
            ))?;
        let a = monitor.area();
...
impl Screens {
    ...
    pub fn ui(
        ...
        crate::kit::toolbar(ui, |ui| {
            if ui
                .add(crate::kit::button(cfg.locale.text("Atualizar", "Refresh")))
                .clicked()
            {
                self.layout.start(move || scan_for(locale));
            }
            ...
            if ui
                .checkbox(
                    &mut cfg.screen_snap,
                    cfg.locale.text("Encaixar na grade", "Snap to grid"),
                )
                .changed()
            {
                out.push(ScreenOut::SaveCfg);
            ...
            if ui
                .add_enabled(
                    ...
                    egui::Button::new(
                        cfg.locale
                            .text("Organizar selecionadas", "Arrange selected"),
                    ),
                )
                .clicked()
            {
                ...
                if response.drag_stopped() {
                        ...
                        (self.dragging.take(), ui.ctx().pointer_latest_pos())
                    {
                            ...
                            .cloned()
                        {
                            ...
                            self.action
                                .start(move || place_for(&address, &m, target, locale));
                        }
                    }
                }
            }
        ...
        crate::kit::toolbar(ui, |ui| {
            ...
            ui.add(
                egui::TextEdit::singleline(&mut self.preset)
                    .hint_text(cfg.locale.text("nome", "name"))
                    .desired_width(160.0),
            );
            if ui
                .add_enabled(
        ...
        for (name, preset) in &mut cfg.screen_presets {
            ui.collapsing(name, |ui| {
                if ui
                    .add_enabled(
                        !self.action.busy(),
                        crate::kit::primary(cfg.locale.text("Aplicar cenário", "Apply scene")),
                    )
                    .clicked()
                {
                ...
                for slot in &mut preset.slots {
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .checkbox(
                                &mut slot.launch,
                                cfg.locale.text("Abrir se fechada", "Open if closed"),
                            )
                            .changed()
                        {
                            out.push(ScreenOut::SaveCfg);
                            ...
                            ui.with_layout(
                                ...
                                |ui| {
                                    if window.floating
                                        && ui
                                            .add(crate::kit::button(
                                                cfg.locale.text("Voltar ao mosaico", "Tile window"),
                                            ))
                                            .clicked()
                                    {
                                        ...
                                        self.action.start(move || {
                                            dispatch_for(
                                                "settiled",
                                                &selector_for(&a, locale)?,
                                                locale,
                                            )
                                        });
                                    }
                                    if ui
                                        .add(crate::kit::button(cfg.locale.text("Focar", "Focus")))
                                        .clicked()
                                    {
                                        let a = window.address.clone();
                                        self.action.start(move || {
                                    ...
                                    if window.floating {
                                        crate::kit::badge(
                                            ui,
                                            cfg.locale.text("flutuando", "floating"),
                                            crate::app::ACCENT_FG,
                                        );
                                    }
                                },
...
mod linux_integration {
    ...
    fn window_move_resize_restore() {
        ...
        for _ in 0..2 {
            ...
            assert!(
                (moved.at[0] - target.left()).abs() < 4.0
                    && (moved.at[1] - target.top()).abs() < 4.0,
                "{:?} != {:?}",
                moved.at,
                target
            );
