...
impl Drains {
    ...
    pub fn snapshot_json(&mut self) -> serde_json::Value {
        ...
        let services = inventory
            ...
            .into_iter()
            .filter_map(|entry| {
                let Source::Unit { .. } = entry.source else { return None };
                Some(json!({
                    "name": entry.name,
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui, _procs: &[ProcInfo], _admin: bool, locale: Locale) -> Vec<DrainOut> {
        use crate::kit;
        self.scan.poll();
        if self.action.poll() {
        ...
        ui.add_space(6.0);
        kit::toolbar(ui, |ui| {
            ui.checkbox(&mut self.only_active, RichText::new(locale.text("Somente em execução", "Running only")).size(12.5));
            if ui.add(kit::button(locale.text("Atualizar", "Refresh"))).clicked() {
                self.scan.start(startup_linux::scan);
            }
            self.scan.status(ui);
                ...
                kit::row(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            if let Source::Unit { user, unit } = &e.source {
                                if ui
                                    .add_enabled(
                                        ...
                                        kit::button(if e.enabled { locale.text("Não iniciar no boot", "Do not start at boot") } else { locale.text("Iniciar no boot", "Start at boot") }),
                                    )
                                    .clicked()
                                {
                                    let e = e.clone();
                                ...
                                for (label, action, enabled) in [(locale.text("Iniciar", "Start"), "start", !e.active), (locale.text("Reiniciar", "Restart"), "restart", e.active), (locale.text("Parar", "Stop"), "stop", e.active)] {
                            ...
                            if e.active {
                                kit::badge(ui, locale.text("em execução", "running"), Color32::from_rgb(120, 200, 140));
                            }
                            ui.label(
                                RichText::new(e.memory.map(|m| format!("{:.1} MiB", m as f64 / 1048576.0)).unwrap_or_else(|| locale.text("RAM indisponível", "RAM unavailable").into()))
                                    .monospace()
                                    .size(12.5),
                            );
                        });
