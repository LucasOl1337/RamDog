...
impl BootGroup {
    ...
    pub fn label_for(self, locale: Locale) -> &'static str {
        match self {
            BootGroup::StatusPhase => locale.text("Sobe / não sobe → fase", "Starts / does not start → phase"),
            BootGroup::StatusKind => locale.text("Sobe / não sobe → tipo", "Starts / does not start → source"),
...
impl MemMetric {
    ...
    pub fn label_for(self, locale: Locale) -> &'static str {
        match self {
            ...
            MemMetric::WorkingSet => if cfg!(target_os = "linux") { locale.text("Residente (RSS)", "Resident (RSS)") } else { "Working set" },
    ...
    pub fn short_for(self, locale: Locale) -> &'static str {
        match self {
            ...
            MemMetric::Commit => if cfg!(windows) { locale.text("Commit", "Commit") } else { locale.text("Virtual", "Virtual") },
        }
    }
...
mod tests {
    ...
    fn locale_round_trip_and_legacy_default() {
        ...
        assert_eq!(serde_json::from_value::<Config>(value).unwrap().locale, Locale::Portuguese);
    }
