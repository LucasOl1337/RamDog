...
fn main() -> eframe::Result<()> {
    #[cfg(target_os = "linux")]
    {
        if std::env::args().nth(1).as_deref() == Some("--clean-helper") {
            ...
            if let Err(error) = clean::helper(&op) {
                eprintln!("{error}");
                std::process::exit(1);
            }
            return Ok(());
        ...
        if std::env::args().nth(1).as_deref() == Some("--fan-helper") {
            let result = std::env::args()
                ...
                .ok_or_else(|| "PID inválido".to_string())
                .and_then(fans_linux::helper);
...
fn run() -> eframe::Result<()> {
    ...
    if std::env::args().any(|a| a == "--diagnose") {
        ...
        let windows = screens::scan();
        let startup = startup_linux::scan();
        let hw = hwtemp::HwTempReader::spawn()
            .map(|r| r.read())
            .unwrap_or_default();
        println!(
            "{}",
