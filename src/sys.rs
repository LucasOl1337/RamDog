...
pub const SERVICES: &[SvcEntry] = &[
    SvcEntry { name: "WSearch", label: "Windows Search (indexer)", why: "Indexes files continuously (SearchIndexer.exe). Without it, Explorer/Start search is slower — everything else is unchanged.", proc_hint: "searchindexer.exe", stop_only: false },
    SvcEntry { name: "SysMain", label: "SysMain (Superfetch)", why: "Preloads apps into RAM \"just in case.\" On an SSD it is optional and competes for memory.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "DiagTrack", label: "Telemetry (Connected User Experiences)", why: "Collects and sends usage data to Microsoft. No local function depends on it.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "dmwappushservice", label: "WAP Push telemetry", why: "Routes WAP messages for telemetry. Optional.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "DoSvc", label: "Delivery Optimization", why: "Shares Windows/Store updates with other PCs (P2P). Uses RAM/network; updates still download directly without it.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "WerSvc", label: "Windows Error Reporting", why: "Packages and sends crash dumps to Microsoft (WerFault.exe).", proc_hint: "werfault.exe", stop_only: false },
    SvcEntry { name: "MapsBroker", label: "Offline maps", why: "Map download manager for the Maps app. Optional if you do not use it.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "PhoneSvc", label: "Phone Service (Phone Link)", why: "Phone Link support.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "XblAuthManager", label: "Xbox Live Auth", why: "Xbox Live login. Only matters if you use Xbox games/apps.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "XblGameSave", label: "Xbox Live Game Save", why: "Syncs Xbox game saves.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "XboxNetApiSvc", label: "Xbox Live Networking", why: "Networking for Xbox Live games.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "lfsvc", label: "Geolocation", why: "Location service. Apps requesting your location will stop working.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "RemoteRegistry", label: "Remote Registry", why: "Allows editing your registry over the network. Better disabled.", proc_hint: "svchost.exe", stop_only: false },
    SvcEntry { name: "Fax", label: "Fax", why: "It is 2026.", proc_hint: "fxssvc.exe", stop_only: false },
    SvcEntry { name: "wuauserv", label: "Windows Update (stop only)", why: "Windows Update engine (TiWorker/MoUsoCoreWorker come from here). Windows restarts it automatically; stopping it only provides temporary relief during heavy work.", proc_hint: "tiworker.exe", stop_only: true },
];
...
pub fn stop_service(name: &str) -> Result<(), String> {
    ...
    unsafe {
        ...
        ControlService(svc.0, SERVICE_CONTROL_STOP, &mut st).map_err(|e| {
            let c = e.code().0 as u32 & 0xFFFF;
            match c {
                1062 => "already stopped".to_string(),
                1051 => "other services depend on it — stop them first".to_string(),
                1052 | 1061 => "the service cannot be stopped right now".to_string(),
                _ => e.message(),
            }
        })
    }
...
pub fn set_start_type(name: &str, start: SvcStart) -> Result<(), String> {
    ...
    let st: SERVICE_START_TYPE = match start {
        ...
        SvcStart::Unknown => return Err("invalid start type".into()),
    };
    unsafe {
...
pub(crate) fn reg_delete_value(root: HKEY, path: &str, name: &str) -> Result<(), String> {
    let k = reg_open(root, path, true)
        .ok_or_else(|| "access denied — administrator privileges required".to_string())?;
    unsafe {
        let w = wide(name);
...
pub const APPX: &[AppxEntry] = &[
    AppxEntry {
        ...
        why: "Taskbar widgets/news panel. Keeps Widgets.exe + WebView2 alive.",
    ...
    AppxEntry {
        ...
        label: "Phone Link",
        why: "Phone mirroring. Runs PhoneExperienceHost.exe in the background.",
    ...
    AppxEntry {
        ...
        label: "Xbox Game Bar",
        why: "Win+G overlay and game recording. GameBar.exe + GameBarPresenceWriter.",
        procs: &[
            "gamebar.exe",
    ...
    AppxEntry {
        ...
        label: "Copilot (app)",
        why: "Windows Copilot app (WebView2).",
        procs: &["copilot.exe"],
    ...
    AppxEntry {
        ...
        label: "Cortana",
        why: "Legacy assistant with no current Windows 11 function.",
        procs: &["cortana.exe", "searchapp.exe"],
        pkg_name: "Microsoft.549981C3F5F10",
    },
    AppxEntry {
        family_prefix: "MicrosoftTeams",
        label: "Teams (personal, integrated)",
        why: "Consumer Teams included with Windows and started automatically.",
    ...
    AppxEntry {
        ...
        why: "News app with background tasks.",
        procs: &[],
    ...
    AppxEntry {
        ...
        why: "Preinstalled ad-supported games.",
        procs: &[],
...
fn run_elevated_ps_blocking(script: &str) -> Result<(), String> {
    ...
    unsafe {
        ...
        ShellExecuteExW(&mut sei).map_err(|e| {
            let c = e.code().0 as u32 & 0xFFFF;
            if c == 1223 {
                "UAC prompt canceled".to_string()
            } else {
                e.message()
            }
        })?;
        if sei.hProcess.is_invalid() {
            return Err("could not start elevated PowerShell".into());
        }
        ...
        if code == 0 {
            ...
            Err(format!("PowerShell exited with code {code} (Tamper Protection or policy may have blocked it)"))
        }
    }
}
